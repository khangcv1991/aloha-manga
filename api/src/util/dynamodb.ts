...
export const queryByKey = async <T>(params: queryByKeyParams): Promise<T[]> => {
  try {
    ...
    do {
      ...
      lastKey &&
      (params.limit ? allRecords.length < params.limit : true)
    );

    return params.limit ? allRecords.slice(0, params.limit) : allRecords;
  } catch (error) {
    const message = getErrorMessage(error);
    console.error('Failed to complete query');
    throw new Error(message);
...
interface QueryByHashAndRangeParams {
  dynamoDb: DocumentClient;
  table: string;
  keyName: string;
  keyValue: unknown;
...
export const queryByHashAndRange = async <T>(
  params: QueryByHashAndRangeParams,
): Promise<T[]> => {
  try {
    const queryParams: DocumentClient.QueryInput = {
      TableName: params.table,
      ...
      ExpressionAttributeNames: {
        '#a': params.keyName,
        '#c': params.rangeKeyName,
      },
      ExpressionAttributeValues: {
        ':b': params.keyValue,
        ':d': params.rangeKeyValue,
      },
    };
    if (params?.indexName) {
      queryParams.IndexName = params?.indexName;
